...
public class TreeNodeContent {
	 ...
	 * @param trainWeight the total weight of the data distributed on the tree node
	 * @param trainClassDistribution the class distribution of the data distributed
	 *                    on the tree node
	 * @param classification the classification of the tree node
	 * @param errorAsLeafNode the classification error of the tree node as a leaf node
	 */
	...
	public boolean satisfyLeafNode(float minKnownWeight) {
		if(errorAsLeafNode == 0 || trainWeight < 2 * minKnownWeight) return true;
		else return false;
	}
	...
	public String toString() {
		...
		if(errorAsLeafNode > 0) formatter.format("/%.1f", errorAsLeafNode);
		sb.append(")");
		return sb.toString();
	}
}